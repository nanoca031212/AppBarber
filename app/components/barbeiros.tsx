    ...
    <div className="py-6 space-y-6">
      <div>
        ...
        <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory no-scrollbar pb-1">
          ...
          {barbers.map((b) => (
            <Link key={b.id} href={`/barbeiro?id=${b.id}`} className="shrink-0 snap-start">
              <div className="w-40 h-44 rounded-xl bg-[#afafaf] relative overflow-hidden flex flex-col justify-end">
                {b.photo ? (
                  <img src={b.photo} alt={b.name} className="absolute inset-0 w-full h-full object-cover" />
                ) : (
                  <div className="absolute inset-0 flex items-center justify-center bg-zinc-300">
                    <span className="text-3xl font-bold text-white">{b.initials}</span>
                  </div>
                )}
                <div className="relative bg-gradient-to-t from-black/70 to-transparent px-3 py-2">
                  ...
                  {b.description && (
                    <p className="text-white/70 text-xs truncate">{b.description}</p>
                  )}
                </div>
              </div>
            </Link>
          ...
          {services.map((s) => (
            ...
            >
              ...
              <div>
                ...
                {s.description && (
                  <p className="text-xs text-[#656565] leading-snug mt-0.5 line-clamp-2">{s.description}</p>
                )}
